...
4. Next, create a `links` query to return all the `Links` objects
   ...
   2. Each resolver has 4 args
      1. `parent` - (or 'root') the result of the _previous_ resolver. Queries can be nested (inside the {}), so the
         ...
         This is used if a query has multiple steps to get info, i.e. need to look at the Link then User tables - it must get the Link info, and pass that result onto the next stage - querying the User.
      2. `args` - stuff passed into the resolver, i.e. later we use filters, pagination, etc
      ...
      4. `info` - field specific info, rarely used
   3. It’s worth noting that a GraphQL server has built-in default resolvers, so you don’t have to specify a resolver function for every field. A default resolver will look in root to find a property with the same name as the field
...
query, then fetching the Link, then looking up the User name from that Link.
...
- create a `context.ts` file to hold out context obj
...
2. re-generate the _database_ schemas with `npx prisma migrate dev --name "add-vote-relation"`
3. create `graphql/Vote` type and mutator
4. regenerate the _graphql_ schema with `npm run generate` (and check out `schema.graphql` to see the new vote API)
...
4. regenerate the _graphql_ schema with `npm run generate` (and check out `schema.graphql` to see the new scalar, and field in Link)
...
I _assume_ this is a SQL-style filter, where only the data required is returned, rather than a dynamoDB-style filter where it's _all_ returned and filtered on the server.
...
2. Cursor-based. Better suited for non-static content, or infinite scroll, where the items don't always stay in the same order. Here, each item has some unique ID that can be referenced (the `cursor`), and we simply ask for x number of items _after_ that. So for SQL, using `created_at` as our cursor:
...
**No idea if this is good practice** - I extracted the contents of the `Link.fetchAllLinks` resolver into a separate function and imported that into `Feed`'s resolver. Once I'd added the optional args to the `Feed.LinkFeedQuery` query, it just works!
So I can change how I retrive Links and Feed should just work.
